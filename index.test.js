...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number', async () => {
    const testedNumber = await utils.isEvenNumberAsync(2)
    expect(testedNumber).toBe(true)
  })
  test('[20] resolves false if passed an odd number', async () => {
    const testedNumber = await utils.isEvenNumberAsync(3)
    expect(testedNumber).toBe(false)
  })
})
